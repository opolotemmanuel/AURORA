        ...
        <ul className="mt-4 space-y-2 text-sm">
          {stats.recentActivity.length === 0 ? (
            ...
            stats.recentActivity.map((entry, i) => (
              <li key={i} className="flex justify-between gap-4 border-b border-border py-2 last:border-0">
                <span className="capitalize text-muted-foreground">
                  {entry.reason.replace(/_/g, " ")}
                </span>
                <span className={entry.delta > 0 ? "text-foreground" : "text-muted-foreground"}>
                  {entry.delta > 0 ? "+" : ""}
